...
const InsertZohoError_1 = __importDefault(
  require("../../errorHandling/InsertZohoError"),
);
const ZohoGenericError_1 = __importDefault(
  require("../../errorHandling/ZohoGenericError"),
);
const ZohoNotFoundRecords_1 = __importDefault(
  require("../../errorHandling/ZohoNotFoundRecords"),
);
...
class ZohoApi {
  ...
  async updateToken() {
    if (
      !__classPrivateFieldGet(this, _ZohoApi_credentials, "f").refreshToken ||
      ...
      throw new ZohoGenericError_1.default(
  ...
  async updateTokenWithRetry(attempt, attemptNumber) {
    while (
      __classPrivateFieldGet(this, _ZohoApi_accessToken, "f") == null &&
  ...
  async deleteAllRecordsNFeTest(query) {
    ...
    try {
      ...
      if (
        ...
        console.error(error.response?.data);
        throw new ZohoNotFoundRecords_1.default(
    ...
    try {
      ...
      for await (const id of ids) {
        ...
        const result = await __classPrivateFieldGet(
          ...
          _ZohoApi_axios,
          "f",
        ).patch(url, content, requestOptions);
        const data = result.data;
  ...
  async getRecordByField(reportName, field) {
    ...
    const result = await __classPrivateFieldGet(this, _ZohoApi_axios, "f").get(
      url,
      requestOptions,
    );
    return result;
  ...
  async insertRecord(content, config, attemptsNumber) {
    ...
    try {
      const res = await __classPrivateFieldGet(this, _ZohoApi_axios, "f").post(
        url,
        content,
        requestOptions,
      );
      ...
      if (ZohoApi.isInvalidResponse(res)) {
        throw new InsertZohoError_1.default(
      ...
      if (parsed.success) {
        ...
        throw new Error(
          `Zod error, verifique o nome da tabela: ${JSON.stringify(parsed.error)}`,
        );
      }
...
((_ZohoApi_axios = new WeakMap()),
  (_ZohoApi_accessToken = new WeakMap()),
  (_ZohoApi_credentials = new WeakMap()));
