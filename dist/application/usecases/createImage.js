...
const QiveApiError_1 = __importDefault(require("../../infra/errorHandling/QiveApiError"));
const html_pdf_node_1 = __importDefault(require("html-pdf-node"));
class CreateImage {
    constructor(pdfOptions = { format: "A4" }) {
        this.pdfOptions = pdfOptions;
    }
    async renderizarNotaNfe(nota) {
        try {
            const htmlText = this.buildNFeTemplate(nota);
            const options = { format: "A4" };
            const file = { content: htmlText };
            const pdfBuffer = html_pdf_node_1.default.generatePdf(file, options);
            return pdfBuffer;
        }
        catch (error) {
            throw new QiveApiError_1.default("Erro ao renderizar arquivo da NFe", JSON.stringify(error));
        }
    }
    async renderizarNotaNfse(nota) {
        try {
            ...
            throw new QiveApiError_1.default("Erro ao renderizar arquivo da NFSe", JSON.stringify(error));
        }
    }
    buildNFeTemplate(nota) {
    ...
    <div class="detalhe-documento-nfe">
            ...
            <div class="nfe-secao-conteudo">
              <div class="nfe-grid uma-coluna">
                <div class="nfe-campo">
                  ...
                  <span class="valor destaque">${this.safeValue(nota.emit_xNome)}</span>
                </div>
              </div>
              <div class="nfe-grid tres-colunas">
                <div class="nfe-campo">
                  <span class="label">CNPJ/CPF</span>
                  <span class="valor">${this.formatCNPJ(nota.CNPJ_Emitente)}</span>
                </div>
                <div class="nfe-campo">
              ...
              <div class="nfe-grid uma-coluna">
                <div class="nfe-campo">
                  ...
                  <span class="valor">${this.safeValue(nota.emit_xLgr)}, ${this.safeValue(nota.emit_nro)} - ${this.safeValue(nota.emit_xBairro)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="nfe-secao">
            ...
            <div class="nfe-secao-conteudo">
              <div class="nfe-grid uma-coluna">
                <div class="nfe-campo">
                  <span class="label">Nome/Razão Social</span>
                  <span class="valor destaque">${this.safeValue(nota.dest_XNome)}</span>
                </div>
              </div>
              <div class="nfe-grid tres-colunas">
                <div class="nfe-campo">
                ...
                <div class="nfe-campo">
                  ...
                  <span class="valor">${this.formatDataHora(nota.ide_dhEmi)}</span>
                </div>
                <div class="nfe-campo">
                  <span class="label">Tipo de Operação</span>
                  <span class="valor">${nota.ide_tpNF === "1" ? "SAÍDA" : "ENTRADA"}</span>
                </div>
              </div>
              <div class="nfe-grid uma-coluna">
                <div class="nfe-campo">
                  <span class="label">Endereço</span>
                  <span class="valor">${this.safeValue(nota.dest_xLgr)}, ${this.safeValue(nota.dest_nro)} - ${this.safeValue(nota.dest_xBairro)}, ${this.safeValue(nota.dest_xMun)} - ${this.safeValue(nota.dest_UF)}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="nfe-totais-grupo">
              ...
              <div class="nfe-totais-conteudo">
                <div class="nfe-totais-linha">
                  ...
                  <span class="nfe-totais-valor">${formatCurrency(nota.vProd)}</span>
                </div>
                <div class="nfe-totais-linha">
                  <span class="nfe-totais-label">Valor do ICMS:</span>
                  <span class="nfe-totais-valor">${formatCurrency(nota.vICMS)}</span>
                </div>
                <div class="nfe-totais-linha">
                  <span class="nfe-totais-label">Valor do IPI:</span>
                  <span class="nfe-totais-valor">${formatCurrency(nota.vIPI)}</span>
                </div>
                <div class="nfe-totais-linha">
            ...
            <div class="nfe-totais-grupo">
              ...
              <div class="nfe-totais-conteudo">
                ...
                <div class="nfe-totais-linha">
                  ...
                  <span class="nfe-totais-valor">${formatCurrency(nota.vFrete)}</span>
                </div>
                <div class="nfe-totais-linha">
                  <span class="nfe-totais-label">Valor do Seguro:</span>
                  <span class="nfe-totais-valor">${formatCurrency(nota.vSeg)}</span>
                </div>
                <div class="nfe-totais-linha">
                  <span class="nfe-totais-label">Desconto:</span>
                  <span class="nfe-totais-valor">${formatCurrency(nota.vDesc)}</span>
                </div>
                <div class="nfe-totais-linha">
                  <span class="nfe-totais-label">Outras Despesas:</span>
                  <span class="nfe-totais-valor">${formatCurrency(nota.vOutro)}</span>
                </div>
                <div class="nfe-totais-linha">
                  <span class="nfe-totais-label">VALOR TOTAL DA NFe:</span>
                  <span class="nfe-totais-valor">${formatCurrency(nota.vNF)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="nfe-info-adicional">
            ...
            <div class="nfe-info-adicional-conteudo">
              <strong>Natureza da Operação:</strong> ${this.safeValue(nota.ide_natOp)}<br>
          ...
          <div class="nfe-rodape">
            <div>
              <strong>NFe nº ${this.safeValue(nota.ide_nNF)} - Série ${this.safeValue(nota.ide_serie)}</strong><br>
          ...
          <div class="wrapper-nfse-cabecalho">
            ...
            <div class="nfse-info-principais">
              ...
              <div class="nfse-info-item">
                ...
                <span class="valor">${this.formatDataHora(nota.DataEmissao)}</span>
              </div>
              <div class="nfse-info-item">
                <span class="label">Código de Verificação</span>
                <span class="valor">${this.safeValue(nota.CodigoVerificacao)}</span>
              </div>
            </div>
          </div>
            ...
            <div class="nfse-secao-conteudo">
              <div class="nfse-grid-campos">
                <div class="nfse-campo">
                  ...
                  <span class="valor destaque">${this.formatCNPJ(nota.PrestadorServicoCnpj)}</span>
                </div>
                <div class="nfse-campo">
              ...
              <div class="nfse-grid-campos uma-coluna">
                <div class="nfse-campo">
                  <span class="label">Nome/Razão Social:</span>
                  <span class="valor destaque">${this.safeValue(nota.PrestadorServicoRazaoSocial)}</span>
                </div>
              </div>
              <div class="nfse-grid-campos uma-coluna">
                <div class="nfse-campo">
          ...
          <div class="nfse-secao">
            ...
            <div class="nfse-secao-conteudo">
              <div class="nfse-grid-campos">
                <div class="nfse-campo">
                  <span class="label">CPF/CNPJ:</span>
                  <span class="valor destaque">${this.formatCNPJ(nota.TomadorCnpj)}</span>
                </div>
                <div class="nfse-campo">
              ...
              <div class="nfse-grid-campos uma-coluna">
                <div class="nfse-campo">
                  <span class="label">Nome/Razão Social:</span>
                  <span class="valor destaque">${this.safeValue(nota.TomadorRazaoSocial)}</span>
                </div>
              </div>
              <div class="nfse-grid-campos uma-coluna">
                <div class="nfse-campo">
